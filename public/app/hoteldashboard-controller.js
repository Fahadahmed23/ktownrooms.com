app.controller('hotelDashboardCtrl', function($scope, DTColumnDefBuilder, DTOptionsBuilder) {
    ...
    $scope.getCheckins = function(period) {
        ...
        $scope.ajaxGet('getCheckins', { period: period, hotel_id: $scope.selected_hotel_id }, true)
    ...
    $scope.getCheckouts = function(period) {
        ...
        $scope.ajaxGet('getCheckouts', { period: period, hotel_id: $scope.selected_hotel_id }, true)
            .then(function(response) {
    ...
    $scope.getRecords = function(h_id) {
        $scope.getCheckouts('as of today');
        $scope.getCheckins('as of today');
        $scope.selected_hotel_id = h_id;

        setTimeout(() => {
            
            $scope.ajaxGet('get_hotel_records', {
                    hotel_id: h_id
                }, true)
                .then(function(response) {
                    ...
                    if (h_id == undefined) {
                        $scope.hotels[0].is_active = true;
                        $scope.selected_hotel_id = $scope.hotels[0].id;
                    }
    
                    if (h_id) {
                        $scope.hotels = $scope.hotels.map(h => {
                            if (h.id == h_id) {
                                h.is_active = true;
                            } else {
                                h.is_active = false;
                    ...
                    */
                    ...
                    setTimeout(() => {
                        ...
                        $('#pie_progress_bar').html('');
                        $('#chart_area_basic').html('');
                        $('#progress_icon_four').html('');
                        $('#progress_icon_three').html('');
