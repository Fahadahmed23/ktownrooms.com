...
class ReportControllerTwo extends Controller
{
  ...
  public function index_reports_get_cash_flow_cashin()
  {
    \Session::forget('breadcrumb');
    return view('reports_get_cash_flow_cashin.index');
  }

  public function index_reports_get_cash_flow_cashout()
  {
    \Session::forget('breadcrumb');
    return view('reports_get_cash_flow_cashout.index');
  }
...
// Reports index views Arman Ahmad 19-March-2022 - End
  ...
  */
  ...
  public function get_sales_summary_report(Request $request)
  {
    ...
    for($date = $date_from_dt; $date <= $date_to_dt; $date->modify('+1 day')) {
      ...
      **/
      ...
      $rooms_occupied1 = Booking::with(['hotel','rooms', 'rooms.category','services','booking_miscellaneous_amount','invoice','invoice_details','promotion','tax_rate', 'invoice.payment_mode'])
      ->where('hotel_id',$hotel_id)
      ->whereIn('status', ['CheckedIn','CheckedOut'])
      ->whereBetween('checkin_time', [$date_one,$date_two_next])
      ->orderBy('created_at', 'desc')->get();

      $rooms_occupied2 = Booking::with(['hotel','rooms', 'rooms.category','services','booking_miscellaneous_amount','invoice','invoice_details','promotion','tax_rate', 'invoice.payment_mode'])
      ->where('hotel_id',$hotel_id)
      ->where('status', 'CheckedIn')
      ->where('BookingFrom', '<=', $loop_date)
      ->where('BookingTo', '>=', $loop_date)
      ...
      $rooms_occupied = $rooms_occupied1->merge($rooms_occupied2);
  ...
  public function get_cash_flow_cashin_report(Request $request) {
    ...
    //for($date = $date_from_dt; $date <= $date_to_dt; $date->modify('+1 day')) {
    for($date = $date_to_dt; $date >= $date_from_dt; $date->modify('-1 day')) {
      ...
      **/
      ...
      /*
      $booking1 = Booking::with(['hotel','rooms', 'rooms.category','services','invoice','invoice_details','tax_rate','created_by_user'])
      ->where('hotel_id',$hotel_id)
      ->whereIn('status', ['CheckedIn','CheckedOut'])
      ->whereBetween('checkin_time', [$date_one,$date_two_next])
      ->orderBy('created_at', 'desc')->get();

      $booking2 = Booking::with(['hotel','rooms', 'rooms.category','services','invoice','invoice_details','tax_rate','created_by_user'])
      ->where('hotel_id',$hotel_id)
      ->where('status', 'CheckedIn')
      ->whereIn('status', ['CheckedIn','CheckedOut'])
      ->where('BookingFrom', '<=', $loop_date)
      ...
      **/
       ...
       if(!empty($bookings)){
        ...
        if($count > 0) {
          ...
          $bookings_map = $bookings->map(function ($ex) {
            ...
            $obj->invoice_details = $ex->invoice_details->map(function ($invoice_detail) {

              $obj['amount'] = $invoice_detail->amount;
              $obj['created_at'] = $invoice_detail->created_at;
              $obj['created_by_user'] = $invoice_detail->created_by_user->name;
              return $obj;
            });

            //$obj->invoice_details = $ex->invoice_details;
          ...
          if(count($bookings_map) > 0){
            foreach($bookings_map as $bookings_map_single) {
              ...
              foreach($bookings_map_single->invoice_details as $single_invoicedetail){
                  $inner_id_arr = array();
                  ...
                  $inner_id_arr['checkin_time'] = $bookings_map_single->checkin_time;
                  $inner_id_arr['checkout_time'] = $bookings_map_single->checkout_time;
                  $inner_id_arr['BookingDate'] = $single_invoicedetail['created_at'] ?? "";
                  $inner_id_arr['BookingFrom'] = $bookings_map_single->BookingFrom;
                  ...
                  $invoice_details_arr[] = $inner_id_arr;
      ...
      $objj_opening_balance->id = 0000;
      $objj_opening_balance->booking_date = $loop_date;
      $objj_opening_balance->opening_balance = $user_opening_balance;



      $bookings_exec_array[] = $objj_opening_balance;
    
      foreach($bookings_exec as $single_bookings_exec){

        $bookings_exec_array[] = $single_bookings_exec;
      }

      $objj_cash_in = new stdClass();
      $objj_cash_in->id = 1111;
      $objj_cash_in->booking_date = $loop_date;
      $objj_cash_in->cash_in = $total_amount_received;

      $bookings_exec_array[] = $objj_cash_in;
    ...
    foreach($get_cash_flow_arr as $single_get_cash_flow){
      ...
      foreach($single_get_cash_flow as $single_one) {
        $get_cash_flow_whole[] = $single_one;
      }
  ...
  public function get_cash_flow_cashout_report(Request $request) {
    ...
    for($date = $date_to_dt; $date >= $date_from_dt; $date->modify('-1 day')) {
      ...
      foreach($vouchers_exec as $single_vouchers_exec) {
        $bookings_exec_array[] = $single_vouchers_exec;
      }
  ...
  public function get_invoice_search(Request $request) {
    ...
    $user_mobile_no = $request->mobile_no ?? null;
  ...
  public function get_expenses_report(Request $request){
    ...
    if(count($get_expenses_report_arr)){
      ...
      return response()->json([
        'success' => true,
        'result' => $get_expenses_report_array,
        'msgtype' => 'success',
      ]);
      ...
      return response()->json([
        'success' => true,
        'result' => $get_expenses_report_arr,
        'msgtype' => 'success',
      ]);
