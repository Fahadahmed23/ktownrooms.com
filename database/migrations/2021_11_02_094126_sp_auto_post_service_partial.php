...
class SpAutoPostServicePartial extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    {
        $procedure = "
            CREATE PROCEDURE `AutoPostServicePartial`(amt INT,pay_type VARCHAR(25), title VARCHAR(100),b_id INT,b_no VARCHAR(50),u_id INT)
            BEGIN
                ...
                IF STRCMP(pay_type, 'payment') = 0 THEN
                    SET @narration = CONCAT('Partial Payment for ');
                    SET @auto_post_title = 'partial payment';
                END IF;
                
                IF STRCMP(pay_type, 'refund') = 0 THEN
                    SET @narration = CONCAT('refundable Payment for ');
                    SET @auto_post_title = 'refund';
                END IF;
                
                IF STRCMP(pay_type, 'service') = 0 THEN
                    SET @narration = CONCAT(title, ' for ');
                    SET @auto_post_title = 'accept service';
                END IF;                                                                                                                                                                                       
                
                -- Get account_auto_posting_type.id for checkedin
                -- SET @type_id = (SELECT id FROM account_auto_posting_type a WHERE a.is_active = 1 AND STRCMP(a.title,@auto_post_title COLLATE utf8_general_ci) LIMIT 1);
                SET @type_id = (SELECT id FROM account_auto_posting_type a WHERE a.is_active = 1 AND a.title=@auto_post_title LIMIT 1);
                ...
                SET @b_no = '';
                SET @hotel_id = 0;
                SELECT hotel_id, booking_no INTO @hotel_id, @b_no FROM bookings WHERE id = b_id LIMIT 1;
        ...
        DB::unprepared("DROP procedure IF EXISTS AutoPostServicePartial");
